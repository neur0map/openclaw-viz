...
export const RightPanel = ({ onFocusNode }: RightPanelProps) => {
  ...
  const tabs = [
    { id: 'processes' as const, label: 'Flows', icon: GitBranch },
  ...
  return (
    ...
    >
      ...
      />
      <div className="flex items-center justify-between px-3 py-2 glass border-b border-white/[0.08]">
        <div className="flex items-center gap-0.5 p-0.5 bg-white/[0.04] rounded-md">
          {tabs.map(({ id, label, icon: Icon }) => (
            <button
              ...
              className={`
                flex items-center gap-1.5 px-2.5 py-1.5 text-[12px] rounded transition-all
                ${activeTab === id
              ...
              if (config) {
                ...
                return (
                  <span className="text-[11px] px-2 py-0.5 rounded-full bg-white/[0.04] border border-white/[0.08] text-text-muted flex items-center gap-1.5">
                    <span className="w-3 h-[3px] rounded-full bg-emerald-400/70" />
                    {modelName}
                  </span>
                );
              }
